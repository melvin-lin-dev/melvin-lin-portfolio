...
interface CompetitionTimelineItemProps {
    competition: Competition;
    isActive: boolean;
}

export default function CompetitionTimelineItem({ competition, isActive }: CompetitionTimelineItemProps): ReactElement {
    ...
    const collapsableId = `${competition.category}-collapsible-${competition.id}`;
    const showDetail = competition.descriptions.length > 4;

    return (
        <>
            <h3 className="sm:text-lg md:text-xl font-semibold">{competition.title}</h3>
            <p className={`!mt-1 text-sm sm:text-base ${style.content.location}`}>{competition.location}</p>
            <p className={`!mt-1 !font-normal text-sm sm:text-base ${style.content.note}`}>
                <strong>Issued by:</strong> {competition.organization}
            </p>
            <p className={`!mt-1 !font-normal text-sm sm:text-base ${style.content.note}`}>
                <strong>Issued date:</strong> {competition.organization}
            </p>
            ...
            {competition.note && (
                <p className={`!mt-2 !font-normal text-sm sm:text-base ${style.content.note}`}>
