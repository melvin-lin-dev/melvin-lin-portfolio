...
export default function WorkExperienceTimelineItem({ workExperience, isActive }: WorkExperienceTimelineItemProps): ReactElement {
    ...
    return (
        <>
            ...
            <p className={`!mt-1 text-sm sm:text-base ${style.content.location}`}>{workExperience.location}</p>
            <div className={`relative mt-3 ${showDetail ? "pb-7" : ""}`}>
                {showDetail && (
                    <>
                        <input type="checkbox" id={collapsableId} className="checked:[&+label_.icon]:rotate-180 checked:[&~ul]:max-h-[600px] sm:checked:[&~ul]:max-h-[400px] lg:checked:[&~ul]:max-h-[300px] checked:[&~ul]:after:h-0" hidden />
                        <label
