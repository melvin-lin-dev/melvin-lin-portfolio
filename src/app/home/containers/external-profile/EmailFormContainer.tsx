...
export default function EmailFormContainer(): ReactElement {
    ...
    const sendEmail = (e: FormEvent<HTMLFormElement>) => {
        ...
        emailjs
            .sendForm(EMAIL_CONFIG.serviceId!, EMAIL_CONFIG.templateId!, form.current, EMAIL_CONFIG.publicKey)
            .then(() => {
    ...
    return (
        <Animate variants={shrinkFadeIn} className="py-8 px-4 bg-white rounded-xl shadow-[0_4px_10px_#ccc] lg:shadow-[0_4px_15px_1px_#ccc]">
            <div className="max-w-md mx-auto">
                <Animate tag="h3" variants={fadeUp} delay={0.25} className="mb-6 text-lg sm:text-xl md:text-2xl font-semibold text-center">
                    Get in Touch
                </Animate>
                ...
                <form ref={form} onSubmit={sendEmail} className="space-y-5">
                    ...
                    <ElevatedButton className="w-full py-3 sm:py-4 rounded-md sm:rounded-lg text-sm sm:text-base" isLoading={isLoading}>Send Message</ElevatedButton>
                </form>
            </div>
        </Animate>
    );
